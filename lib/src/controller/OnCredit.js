...
ipcMain.on('/oncredit/create', (e, form) => {
    ...
    const customerDriver = new CustomerDriver()
    ...
    const driverUpdate = customerDriver.update(
        { taxNumber: parameters.customerDriverTaxNumber },
        { id: parameters.customerDriverId }
    )
    console.log(driverUpdate.toString())
    driverUpdate.then(r => {
        delete parameters.customerDriverTaxNumber
        model.create(parameters)
            .then(rowId => {
                ...
                parameters.amount = parameters.totalPrice * -1
        ...
        e.reply('oncreditCreate', { customerUpdate: 'Şoför TC güncellemesi başarısız.' })
        return false
    })
})

ipcMain.on('/oncredit/createPayment', (e, form) => {
    console.log(form)
    const model = new Oncredit()
    const customerDriver = new CustomerDriver()
    const parameters = Oncredit.setPaymentParams(form)
    const validation = Oncredit.paymentValidate(parameters)
    if (!validation.status) {
        e.reply('paymentCreate', validation)
        return false
    }
    if (parameters.customerDriverId) {
        customerDriver.update(
            { taxNumber: parameters.customerDriverTaxNumber },
            { id: parameters.customerDriverId }
        ).then(r => {
            delete parameters.customerDriverTaxNumber
            delete parameters.sms
            model.createPayment(parameters)
                .then(rowId => {
                    e.reply('paymentCreate', { moneyFlowId: rowId })
                })
                .catch(err => {
                    console.log(err)
                })
        }).catch(err => {
            e.reply('paymentCreate', { customerUpdate: 'Şoför TC güncellemesi başarısız.' })
            return false
        })
    } else {
        delete parameters.customerDriverTaxNumber
...
ipcMain.on('/oncredit/paymentTypeList', (e) => {
    const model = new Oncredit()
    const dbResult = model.getPaymentTypes()
    dbResult
        .then(result => {
            e.reply('paymentTypeList', result)
        })
        .catch(err => {
            console.log(err)
        })
                        ...
                        printer
                            ...
                            .then(() => {
                                printer
                                    ...
                                    .text('T.C:' + dataset[0].driverTaxNumber)
                                    .text(divider)
                                    .size(0, 0)
                                    .align('ct')
                                printer.tableCustom([
...
ipcMain.on('/oncredit/printPayment', (e, data) => {
    const model = new Oncredit()
    model.getPayment(data).then(dataset => {
        const branchId = dataset[0].branchId
        const branch = new Branch()
        branch.get({ branchId }).then(branch => {
            const branchInfo = branch[0]
                        ...
                        printer
                            ...
                            .then(() => {
                                printer
                                    ...
                                    .style('b')
                                    .text('..::TAHSİLAT FiŞi::..')
                                    .text('..::' + dataset[0].paymentTypeName + '::..')
                                    .style('normal')
                                    .text('')
                                    .text(moment().format('DD.MM.YYYY HH:mm') + ' - Fiş No: ' + data.oncreditId)
                                    .text(divider)
                                    ...
                                    .text('Gsm: ' + _.toUpper(dataset[0].customerPhone))
                                    .text('Adres: ' + _.toUpper(dataset[0].customerAddress))
                                    .text('Plaka No: ' + (dataset[0].plate || 'Yok'))
                                    .text('Şoför Adı: ' + (dataset[0].driverName || 'Yok'))
                                    .text('T.C:' + (dataset[0].driverTaxNumber || 'Yok'))
                                    .text(divider)
                                    .size(0, 0)
                                    .align('ct')
                                dataset[0].description = dataset[0].description ? ('Açıklama: ' + dataset[0].description) : 'Açıklama girilmedi.'
                                printer
                                    ...
                                    .align('ct')
                                    .size(1, 1)
                                    .text('Tutar: ' + dataset[0].totalPrice.toFixed(2) + ' TL')
                                    .size(0, 0)
                                    .text('')
                                    .text(divider)
                                    .align('ct')
